...
const ResultsDisplay: React.FC<Props> = ({
    ...
    return text.split('\n').map((line, i) => {
      ...
      return (
        <div
          key={i}
          className={`mb-1 ${
            line.trim().startsWith('-') ? 'pl-4 relative' : ''
          }`}
        >
          {line.trim().startsWith('-') && (
            <span className="absolute left-0 top-2 w-1.5 h-1.5 bg-brand-accent rounded-full opacity-70" />
          )}
          {parts.map((part, j) =>
            j % 2 === 1 ? (
              <strong
  ...
  const handleDownloadPDF = async () => {
    ...
    // Largeur cible (≈ A4, 210 mm @96 DPI)
    const A4_WIDTH_PX = 700;

    window.scrollTo({ top: 0, behavior: 'instant' });

    /* ---------- Overlay pleine page (plus de 100 vh) ---------- */
    ...
    clone.style.cssText =
      'width: 100%; margin: 0; box-shadow: none; background:#fff;';

    /* Forcer la grille 3 colonnes dans le PDF */
    clone
      .querySelectorAll('.md\\:grid-cols-3')
      .forEach((el) => {
    ...
    /* Conversion d’un éventuel bloc sombre → clair (économie d’encre) */
    const darkBg = clone.querySelector('.bg-brand-dark');
    if (darkBg) {
      darkBg.classList.remove('text-white', 'bg-brand-dark');
      darkBg.classList.add(
        'text-slate-900',
        'bg-gray-50',
        'border',
        'border-gray-300',
      ...
      darkBg
        .querySelectorAll('.text-gray-100')
        .forEach((t) => {
          t.classList.remove('text-gray-100');
          t.classList.add('text-slate-700');
        });
    }

    /* Taille fixe pour les graphiques AVANT html2canvas */
    clone
    ...
    /* Options dynamiques : largeur déduite du conteneur */
    const containerWidth = container.offsetWidth;

    const opt = {
      margin: [15, 15, 15, 15],
      filename: `Nexalis_Audit_ROI_${inputs.industry}_${new Date()
...
Gain annuel estimé : ${formatCurrency(
...
Analyse : ${aiInsight ?? ''}`;
  ...
  return (
    <div
      ...
      className="space-y-6 animate-fade-in bg-white md:bg-transparent relative"
    >
      {/* ---------- Barre d’actions (masquée à l’export) ---------- */}
      <div
        id="action-toolbar"
        className="flex justify-end gap-3 print:hidden mb-2"
      >
        <button
          onClick={handleCopy}
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-brand-dark rounded-lg hover:bg-slate-800 transition-colors shadow-sm disabled:opacity-70"
        >
          {isGeneratingPdf ? (
      ...
      <div
        id="report-header"
        className="hidden mb-8 border-b-2 border-brand-dark pb-4 print:block"
      >
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            <Sparkles className="h-6 w-6 text-yellow-400 fill-yellow-400" />
            <h1 className="text-2xl font-bold text-brand-dark uppercase">
              Nexalis Solutions
            </h1>
          </div>
          <span className="text-sm text-gray-500 font-medium">
            Rapport d’opportunité IA
          </span>
        </div>
        <div className="mt-4 text-sm text-gray-600">
          Audit généré le{' '}
          {new Date().toLocaleDateString('fr-FR')} pour une entreprise
          du secteur <b>{inputs.industry}</b> ({inputs.employees}{' '}
          employés).
        </div>
      </div>

      {/* ---------- Alert coût de l’inaction ---------- */}
      <div className="bg-orange-50 border border-orange-100 rounded-xl p-4 flex items-start gap-4 shadow-sm">
        ...
        <div>
          <h4 className="font-bold text-orange-900 text-sm uppercase tracking-wide mb-1">
            Coût de l’inaction
          </h4>
          <p className="text-orange-800 text-sm leading-relaxed">
            Sans modernisation, vos processus actuels vous coûtent
            environ{' '}
            <span className="font-bold underline decoration-orange-300 decoration-2">
              {formatCurrency(monthlyLoss)} chaque mois
            </span>
            .
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-2xl font-extrabold text-brand-dark mt-1">
                {formatNumber(results.totalHoursSaved)} h
              </p>
            </div>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-soft border border-gray-100 ring-2 ring-brand-accent/10 flex flex-col justify-between h-full relative overflow-hidden">
          <div className="absolute top-0 right-0 w-16 h-16 bg-brand-accent/10 rounded-bl-full -mr-4 -mt-4" />
          <div className="flex items-start justify-between mb-4 relative z-10">
            <div>
              <p className="text-xs font-bold text-brand-accent uppercase tracking-wider">
                Économies annuelles
              </p>
              <p className="text-2xl font-extrabold text-brand-accent mt-1">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-soft border border-gray-100 flex flex-col justify-between h-full">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-2xl font-extrabold text-brand-dark mt-1">
                {formatCurrency(results.threeYearRoi)}
              </p>
            </div>
      ...
      <div className="bg-brand-dark rounded-2xl p-6 shadow-card text-white relative overflow-hidden print:bg-gray-50 print:text-black print:border print:border-gray-300">
        <div className="absolute -right-10 -top-10 bg-white/5 w-40 h-40 rounded-full blur-3xl print:hidden" />
        <div className="relative z-10">
          <h3 className="flex items-center gap-2 text-brand-accent font-bold mb-4 uppercase text-xs tracking-widest print:text-brand-dark">
            <Sparkles
              size={14}
              className="text-brand-accent print:text-brand-dark"
            />
          ...
          <div className="text-sm md:text-base leading-relaxed text-gray-100 print:text-gray-800 border-l-4 border-brand-accent pl-4">
            {isAiLoading ? (
              <span className="animate-pulse italic">
                Analyse de vos données en cours par nos modèles…
              </span>
            ) : (
              renderStructuredAnalysis(aiInsight ?? '') ||
      ...
      {/* ---------- CTA (masqué à l’export) ---------- */}
      <div
        id="cta-section"
        className="mt-8 text-center bg-brand-accent/5 rounded-xl p-8 border border-brand-accent/10 print:hidden"
      >
        <h4 className="font-bold text-brand-dark text-lg mb-2">
          Transformez ce potentiel en réalité
        </h4>
        <p className="text-gray-600 mb-6 max-w-lg mx-auto">
          Ces chiffres sont théoriques. Pour une analyse fine de vos
          process et une feuille de route d’implémentation,
          parlons-en.
        </p>
        <a
